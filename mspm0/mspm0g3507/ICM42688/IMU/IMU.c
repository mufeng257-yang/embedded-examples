...
#define GYRO_CAL_DISCARD_SAMPLES  100U
#define GYRO_CAL_SAMPLES          500U
#define GYRO_CAL_SAMPLE_DELAY_MS  2U
...
static void IMU_calibrateGyro(void)
{
	...
	/* Determine zero-rate output before yaw integration starts.
	 * Keep the board still during this roughly 1.2 second interval. */
	for (i = 0; i < GYRO_CAL_DISCARD_SAMPLES; i++)
	{
		bsp_IcmGetRawData(&accval, &gyroval);
		delay_ms(GYRO_CAL_SAMPLE_DELAY_MS);
	}
	...
	gyro_offset[0] = (float)(sum[0] / GYRO_CAL_SAMPLES);
	gyro_offset[1] = (float)(sum[1] / GYRO_CAL_SAMPLES);
	gyro_offset[2] = (float)(sum[2] / GYRO_CAL_SAMPLES);
	Kp = 0.5f;
...
//	
  ...
  now = nowtime;  //读取时间
  /* nowtime counts 100 us ticks. Unsigned subtraction also handles the
   * natural uint32_t wraparound; halfT is half the elapsed sample period. */
  halfT = (float)((uint32_t)(now - lastUpdate)) * 0.00005f;
  lastUpdate = now;	//更新时间
