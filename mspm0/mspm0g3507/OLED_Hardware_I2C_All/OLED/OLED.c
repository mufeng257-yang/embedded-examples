...
//16X16,zulieshi,nixiang,c51
void OLED_show_HZ(uint8_t x, uint8_t y, uint8_t index)
{
    uint8_t col, row;
    uint8_t temp;
    ...
    {
        /* upper 8 rows of this column (reverse bit order: LSB = top) */
        temp = hanzi[index][col * 2];
        for (row = 0; row < 8; row++)
        {
            if (temp & 0x01)
        ...
        /* lower 8 rows of this column (reverse bit order: LSB = top) */
        temp = hanzi[index][col * 2 + 1];
        for (row = 0; row < 8; row++)
        {
            if (temp & 0x01)
                OLED_draw_point(x + col, y + 8 + row, PEN_WRITE);
            else
                OLED_draw_point(x + col, y + 8 + row, PEN_CLEAR);
            temp >>= 1;
        }
    }
}
